...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const fileItems = document.querySelectorAll('.file-item, .file-folder');
            ...
            function handleSelectionAction(actionId, count) {
                ...
                alert(message);
                // Keluar mode seleksi setelah aksi selesai (kecuali Pilih Semua)
            ...
            function toggleSelectionMode(enable) {
                ...
                // Ikon Plus TIDAK berubah.
                menuToggle.querySelector('i').className = 'fas fa-plus'; 
                
                // Ubah ikon tombol BROWSER/DUNIA menjadi HAMBURGER atau kembali
                if (enable) {
            ...
            // --- 3. EVENT LONG PRESS/KLIK (UNTUK SEMUA ITEM) ---
            fileItems.forEach(item => {
                ...
                const startPress = (e) => {
                    ...
                    pressTimer = setTimeout(() => {
                        if (!isSelecting) {
                             ...
                             // TIDAK ADA toggleMenu(true) di sini.
                        }
                ...
                item.addEventListener('touchend', stopPress);
                item.addEventListener('touchcancel', stopPress);
                item.addEventListener('click', normalAction); 
                item.addEventListener('mousedown', startPress);
                item.addEventListener('mouseup', stopPress);
                item.addEventListener('mouseleave', stopPress);
                
                item.addEventListener('contextmenu', (e) => {
                    if (isSelecting) {
                         e.preventDefault(); 
                         // Cegah default browser, tapi tidak panggil modal.
                    }
                });
            });
            ...
            const toggleMenu = (show) => {
                if (show) {
                    ...
                    if (isSelecting) {
                        // Modal Aksi Seleksi (dipanggil oleh Hamburger)
                        menuModal.querySelector('h3').textContent = 'Menu Aksi Seleksi';
                        updateSelectionMenu(); 
                    } else {
                        // Modal Aksi Cepat (dipanggil oleh Tombol Plus)
                        ...
                        `;
                        // Tambahkan event listener untuk item menu cepat
            ...
            browserLink.addEventListener('click', function(e) {
                e.preventDefault();
                if (isSelecting) {
                    // HANYA DI SINI MODAL AKSI SELEKSI DIPANGGIL
                    toggleMenu(true);
                } 
            });
